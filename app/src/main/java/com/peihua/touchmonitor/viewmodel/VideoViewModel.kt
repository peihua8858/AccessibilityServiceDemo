...
class VideoViewModel(application: Application) : BaseQueryViewModel<PhotoHeader>(application) {
    ...
    override val columns: Array<String>
        get() = arrayOf("_data", "date_modified", "duration",  "_display_name", "_size")

    fun requestImages(sortType: Int) {
        request(pictureState) {
            val titleArray = arrayListOf<String>()
            val result =
                queryCursor(QUERY_TYPE_VIDEO) { cursor, result, path, fileName, formatTime, dateTime, fileSize ->
