...
class PictureViewModel(application: Application) : BaseQueryViewModel<PhotoHeader>(application) {
    ...
    fun requestImages(sortType: Int) {
        request(pictureState) {
            ...
            val result = queryCursor(QUERY_TYPE_IMAGE) {cursor, result, path, fileName, formatTime, dateTime, fileSize ->
